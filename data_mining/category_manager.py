...
class CategoryManager():
    ...
    def get_one_hot_by_code(self, code):
        ''' 소분류 code를 1-hot id로 바꿔줌

        :param code: (string) 소분류 code
        :return: (int) 1-hot id
        '''
        code_id = self.get_id_by_code(code) # code -> id
        one_hot_vector = [0 for _ in range(self.small_category_num)] # id -> 1-hot id
        one_hot_vector[code_id] = 1
        return one_hot_vector

    def get_id_by_one_hot(self, one_hot):
        return (int)(torch.argmax(one_hot).data)

    def get_code_by_one_hot(self, one_hot):
        try:
            return self.get_code_by_id((int)(torch.argmax(one_hot).data))
        except:
            try:
                return self.get_code_by_id(numpy.argmax(one_hot, axis=0))
            except:
                raise AssertionError

def unit_test():
    category_manager = CategoryManager.new_category_manager('data/한국표준산업분류(10차)_국문.xlsx')
    print(f'The name of 011 is {category_manager.get_name_by_code("011")}')
    print(f'The mid category of 011 is {category_manager.get_mid_category("011")}')
    print(f'The big category of 011 is {category_manager.get_big_category("011")}')
    print(f'The big category number is {category_manager.big_category_num}')
    print(f'The mid category number is {category_manager.mid_category_num}')
    print(f'The small category number is {category_manager.small_category_num}')
    print(f'The code of id 0 is {category_manager.get_code_by_id(0)}')
    print(f'The id of code 011 is {category_manager.get_id_by_code("011")}')
    print(f'The id of code 011 is {category_manager.get_one_hot_by_code("011")}')
    print(f'{category_manager.get_code_by_one_hot(category_manager.get_one_hot_by_code("011"))} == 011')
